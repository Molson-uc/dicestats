...
max_threshold = (
    200  # they can be tweaked depending on the camera distance, camera angle, ...
)
...
cap = cv2.VideoCapture(
    0
)  # '0' is the webcam's ID. usually it's 0/1/2/3/etc. 'cap' is the video object.
...
while True:
    ...
    keypoints = detector.detect(
        im
    )  # keypoints is a list containing the detected blobs.
    ...
    cv2.imshow(
        "Dice Reader", im_with_keypoints
    )  # display the frame with keypoints added.
    ...
    if counter % 10 == 0:  # enter this block every 10 frames.
        ...
        ):  # if the last 3 readings are the same...
